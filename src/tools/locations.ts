...
export function registerLocationTools(server: McpServer, client: UltimoClient) {
  // Locations
  server.tool(
    "get_locations",
...
Equipment wordt aan locaties gekoppeld voor ruimtelijk beheer.`,
    ...
    async (input) => {
      try {
        const result = await client.getObjects("Location", buildOData(input));
  ...
  server.tool(
    ...
    async (input) => {
      try {
        const result = await client.getObject("Location", input.id, { select: input.select, expand: input.expand });
        return success(result);
      } catch (e: any) {
        return error(e.message);
      }
...
een fysieke vestiging of complex. Alle equipment, locaties, en magazijnen vallen onder een site.`,
    ...
    async (input) => {
      try {
        const result = await client.getObjects("Site", buildOData(input));
        return success(result);
      } catch (e: any) {
        return error(e.message);
      }
...
Wordt ook gebruikt voor sleutelbeheer (welke cilinders/sleutels horen bij deze ruimte).`,
    ...
    async (input) => {
      try {
        const result = await client.getObjects("Space", buildOData(input));
        return success(result);
      } catch (e: any) {
        return error(e.message);
      }
...
Bevat adresgegevens, oppervlakten, vervangingswaarde, en Google Maps integratie (GeocodeX/Y).`,
    ...
    async (input) => {
      try {
        const result = await client.getObjects("Building", buildOData(input));
        return success(result);
      } catch (e: any) {
        return error(e.message);
      }
  ...
  server.tool(
    ...
    async (input) => {
      try {
        const result = await client.getObjects("BuildingFloor", buildOData(input));
        return success(result);
      } catch (e: any) {
        return error(e.message);
      }
  ...
  server.tool(
    ...
    async (input) => {
      try {
        const result = await client.getObjects("BuildingPart", buildOData(input));
        return success(result);
      } catch (e: any) {
        return error(e.message);
      }
