...
export function registerConvenienceTools(server: McpServer, client: UltimoClient) {
...
De response bevat ALLE velden die de API key mag zien (niet alleen de meegegeven velden).`,
    ...
    async (input) => {
      try {
        ...
        const result = await client.createObject("Job", data);
...
overgangen geldig zijn. Bij een ongeldige overgang geeft de API een foutmelding.`,
    ...
    async (input) => {
      try {
        ...
        const result = await client.patchObject("Job", input.id, data);
        return success(result);
      } catch (e: any) {
        return error(e.message);
      }
...
en urenverantwoording.`,
    ...
    async (input) => {
      try {
        const result = await client.createObject("EmployeeLabourLine", input.data);
        return success(result);
      } catch (e: any) {
        return error(e.message);
      }
...
dat de uitgifte registreert. Verlaagt automatisch de voorraad in het magazijn.`,
    ...
    async (input) => {
      try {
        ...
        const result = await client.createObject("Material", data);
        return success(result);
      } catch (e: any) {
        return error(e.message);
      }
    ...
    async (input) => {
      try {
        ...
        const [openJobs, recentIncidents] = await Promise.all([
          client.getObjects("Job", {
            $filter: `Status in (1, 2, 4)${siteFilter}`,
            count: true,
            $top: 1,
            select: "Id",
          }),
          client.getObjects("Incident", {
    ...
    async (input) => {
      try {
        const [jobs, measurements] = await Promise.all([
          ...
          client.getObjects("EquipmentMeasurementPointValue", {
            $filter: `Equipment eq '${input.equipment_id}'`,
            $top: 10,
            $orderby: "RecordCreateDate desc",
          }),
        ...
        if (input.include_costs !== false) {
          try {
            const costs = await client.getObjects("Cost", {
              $filter: `Equipment eq '${input.equipment_id}'`,
              $top: 20,
              $orderby: "RecordCreateDate desc",
              count: true,
...
Verplichte velden bij aanmaken: Context en Status. Standaard: Context=1, Status=1 (Open).`,
    ...
    async (input) => {
      try {
        ...
        const result = await client.createObject("Purchase", data);
        return success(result);
      } catch (e: any) {
        return error(e.message);
      }
...
van de installatiestructuur.`,
    ...
    async (input) => {
      try {
        ...
        const equipment = await client.getObject("Equipment", input.equipment_id, {
          select: "Id,Description,PartOfEquipment,TopOfEquipment,EquipmentType,Status,Location,Site",
        });

        // Get children
        const children = await client.getObjects("Equipment", {
...
Bijv: '%pomp%' vindt alles met 'pomp' in de beschrijving.`,
    ...
    async (input) => {
      try {
        ...
        const result = await client.getObjects(input.object_type, params);
        return success(result);
      } catch (e: any) {
        return error(e.message);
      }
...
en worden automatisch bijgewerkt door Ultimo.`,
    ...
    async (input) => {
      try {
        const result = await client.getObject("Job", input.id, {
...
specifiek equipment-item. Toont welke periodieke onderhoudstaken voor deze asset zijn ingepland.`,
    ...
    async (input) => {
      try {
        const result = await client.getObjects("PmWorkOrder", {
          $filter: `Equipment eq '${input.equipment_id}'`,
          $top: input.top ?? 50,
          count: true,
        });
...
Essentieel voor onderhoudsvoorbereiding en inkoopplanning.`,
    ...
    async (input) => {
      try {
        const result = await client.getObjects("EquipmentSparePart", {
          $filter: `Equipment eq '${input.equipment_id}'`,
          $top: 100,
          count: true,
        });
    ...
    async (input) => {
      try {
        const [equipment, pmWorkOrders] = await Promise.all([
          client.getObject("Equipment", input.equipment_id, {
            select: "Id,Description,Status,EquipmentType,Site,Location,PartOfEquipment",
          }),
          client.getObjects("PmWorkOrder", {
            $filter: `Equipment eq '${input.equipment_id}'`,
            $top: input.top ?? 20,
...
die meer velden kopieert (custom columns, SLA herberekening) — die wordt via de UI getriggered.`,
    ...
    async (input) => {
      try {
        ...
        const original = await client.getObject("Job", input.original_job_id, {
