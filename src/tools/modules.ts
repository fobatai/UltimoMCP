...
export function registerModuleTools(server: McpServer, client: UltimoClient) {
  // ==================== INVOICE ====================
  server.tool(
    "get_invoices",
...
Contexts: 1=Standaard, 2=Fleet. Expand met Lines voor factuurregels.`,
    ...
    async (input) => {
      try {
        const result = await client.getObjects("Invoice", buildOData(input));
...
(Article, Equipment, Job, Project, CostCenter, PurchaseLine, ReceiptLine, ServiceContractLine).`,
    ...
    async (input) => {
      try {
        const result = await client.getObject("Invoice", input.id, { select: input.select, expand: input.expand });
        return success(result);
      } catch (e: any) {
        return error(e.message);
      }
...
Filter op ParentJob om sub-jobs van een specifieke multi-job te vinden.`,
    ...
    async (input) => {
      try {
        ...
        const result = await client.getObjects("Job", params);
        return success(result);
      } catch (e: any) {
        return error(e.message);
      }
...
ShiftLogLine statussen: 1=Open, 2=Doorgeschoven, 4=Gepland, 32=Gesloten, 128=Gereed.`,
    ...
    async (input) => {
      try {
        ...
        const result = await client.getObjects("ShiftLog", {
          ...params,
...
Expand met InspectionLines voor checklist-items, Equipment, Job, ProgressStatusHistories.`,
    ...
    async (input) => {
      try {
        const result = await client.getObject("Permit", input.id, {
...
Expand met ProgressStatusHistories, ShiftLogLine voor context.`,
    ...
    async (input) => {
      try {
        const result = await client.getObject("SafetyIncident", input.id, {
...
Statussen: Concept → Open → Goedgekeurd → Actief → Inactief → Beëindigd.`,
    ...
    async (input) => {
      try {
        const result = await client.getObject("ServiceContract", input.id, {
        ...
        try {
          const result = await client.getObject("ServiceContract", input.id, { expand: "Lines" });
          return success(result);
        } catch (e2: any) {
          return error(e.message);
        }
...
Expand met Occurrences voor de individuele boekingen met start/eindtijd.`,
    ...
    async (input) => {
      try {
        ...
        const result = await client.getObjects("Reservation", {
          ...params,
          expand: params.expand,
        });
        return success(result);
...
Bevat energiebronnen, vergrendelmethoden, en isolatiepunten (via PlotPlan markers).`,
    ...
    async (input) => {
      try {
        const result = await client.getObjects("LockoutTagoutPlan", buildOData(input));
        return success(result);
      } catch (e: any) {
        return error(e.message);
      }
...
aan een job/werkvergunning. Statussen: Open, Aangevraagd, Actief, Geannuleerd.`,
    ...
    async (input) => {
      try {
        const result = await client.getObjects("LockoutTagoutRequest", buildOData(input));
        return success(result);
      } catch (e: any) {
        return error(e.message);
      }
...
Bevat ook notificatie-templates (EmpTextTemplateCode, ManagerTextTemplateCode, etc.).`,
    ...
    async (input) => {
      try {
        const result = await client.getObjects("ProgressStatusFlow", buildOData(input));
        return success(result);
      } catch (e: any) {
...
Alternatief: gebruik JobProgressStatusHistory voor job-specifieke statusgeschiedenis.`,
    ...
    async (input) => {
      try {
        const result = await client.getObjects("ObjectProgressStatusHistory", buildOData(input));
        return success(result);
      } catch (e: any) {
        return error(e.message);
      }
    ...
    async (input) => {
      try {
        const [purchase, lines, receipts] = await Promise.all([
          client.getObject("Purchase", input.purchase_id, {
            expand: "Vendor",
            select: "Id,Description,Status,Vendor,Total,Site,Department",
          }),
          client.getObjects("PurchaseLine", {
            $filter: `Purchase eq '${input.purchase_id}'`,
            $top: 50,
          }),
          client.getObjects("Receipt", {
            $filter: `Purchase eq '${input.purchase_id}'`,
            $top: 20,
          }),
        ]);
...
geboekt op de geregistreerde kostendragers (job OF installatie/afdeling/kostenplaats).`,
    ...
    async (input) => {
      try {
        const result = await client.getObjects("WarehouseServeOut" as string, buildOData(input));
        return success(result);
      } catch (e: any) {
...
(arbeid, materiaal, inkoop, gereedschap, magazijn). Context: 1=Standaard.`,
    ...
    async (input) => {
      try {
        const result = await client.getObjects("InvoiceOut" as string, buildOData(input));
        return success(result);
      } catch (e: any) {
        return error(e.message);
      }
