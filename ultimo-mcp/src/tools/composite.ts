...
beeld wilt van een werkorder zonder meerdere losse calls te doen.`,
    ...
    async (input) => {
      try {
        const job = await getObject("Job", input.id, {
...
meetpunten, reserveonderdelen, storingstypen. Geeft een compleet beeld van een asset.`,
    ...
    async (input) => {
      try {
        const eq = await getObject("Equipment", input.id, {
...
artikelgegevens met voorraadniveaus per magazijn. Handig voor inkoopplanning.`,
    ...
    async (input) => {
      try {
        ...
        const result = await getObjects("ArticleWarehouse", {
          $top: input.top ?? 50,
          $filter: filters.length > 0 ? filters.join(" and ") : undefined,
          count: true,
        });
    ...
    async (input) => {
      try {
        ...
        const promises = input.object_types.map(async (type) => {
          try {
            const data = await getObjects(type, {
              $top: input.top ?? 10,
              $filter: input.filter,
              select: input.select,
              count: true,
