...
import { z } from "zod";
import { executeBatch, getNavigationCollection, getNavigationSingle, getObjectByExternalId, getNextPage, executeAction, getObjects } from "../client.js";
...
Voorbeeld: [{"id":"1","method":"PATCH","url":"object/Equipment('001')","body":{"Description":"Nieuw"}}]`,
    ...
    async (input) => {
      try {
        const result = await executeBatch(input.requests);
...
Niet-werkend: "Equipment('001')/Jobs" — gebruik filter in plaats daarvan.`,
    ...
    async (input) => {
      try {
        const result = await getNavigationCollection(input.path, buildOData(input));
        return success(result);
      } catch (e: any) {
        return error(e.message);
      }
...
om de afdeling van een equipment op te halen. Kan niet gebruikt worden voor collecties.`,
    ...
    async (input) => {
      try {
        const result = await getNavigationSingle(input.path, { select: input.select, expand: input.expand });
        return success(result);
      } catch (e: any) {
        return error(e.message);
      }
...
De combinatie ExternalId + DataProvider is uniek. DataProvider default naar 'Rest'.`,
    ...
    async (input) => {
      try {
        const result = await getObjectByExternalId(
          input.object_type,
          input.external_id,
          input.data_provider,
...
volgende pagina op te halen. Blijf dit herhalen totdat nextPageLink null is.`,
    ...
    async (input) => {
      try {
        const result = await getNextPage(input.next_page_link);
        return success(result);
      } catch (e: any) {
        return error(e.message);
      }
...
Maximum 10 pagina's worden opgehaald (veiligheidsgrens).`,
    ...
    async (input) => {
      try {
        ...
        let result = await getObjects(input.object_type, {
          $filter: input.filter,
          select: input.select,
        ...
        while (result.nextPageLink && page < maxPages) {
          result = await getNextPage(result.nextPageLink);
          allItems.push(...result.items);
          page++;
        }
...
Nested GET werkt ook: Equipment('00001')/SpareParts als alternatief voor expand.`,
    ...
    async (input) => {
      try {
        const { postNested } = await import("../client.js");
        const result = await postNested(input.path, input.data);
        return success(result);
      } catch (e: any) {
        return error(e.message);
      }
...
De ApplicationElementId header is vaak vereist (een GUID uit de Application Element Tree).`,
    ...
    async (input) => {
      try {
        ...
        const result = await executeAction(input.workflow_name, input.data ?? {}, headers);
        return success(result);
      } catch (e: any) {
        return error(e.message);
      }
...
Gebruik PUT voor idempotente upserts (insert-or-update) — ideaal voor integraties.`,
    ...
    async (input) => {
      try {
        const { putObject } = await import("../client.js");
        const result = await putObject(input.object_type, input.id, input.data);
        return success(result);
      } catch (e: any) {
        return error(e.message);
      }
...
is uniek. DataProvider default naar 'Rest'. Gebruikt PUT voor idempotente upserts.`,
    ...
    async (input) => {
      try {
        const dpPart = input.data_provider
          ? `ExternalId='${encodeURIComponent(input.external_id)}',DataProvider='${encodeURIComponent(input.data_provider)}'`
          : `ExternalId='${encodeURIComponent(input.external_id)}'`;

        const { putObject } = await import("../client.js");
        // Use raw request path for ExternalId-based PUT
        const url = `/object/${input.object_type}(${dpPart})`;
        const result = await fetch(`${process.env.ULTIMO_BASE_URL}/api/v1${url}`, {
          method: "PUT",
          headers: {
        ...
        if (!result.ok) {
          ...
          throw new Error(`Ultimo API fout (HTTP ${result.status}): ${body}`);
        }
        const text = await result.text();
        return success(text ? JSON.parse(text) : { success: true });
      } catch (e: any) {
        return error(e.message);
      }
    }
