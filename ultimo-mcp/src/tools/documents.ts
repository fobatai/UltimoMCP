...
import { z } from "zod";
import { getObjects, getObject, createObject, patchObject, executeAction } from "../client.js";
...
die gekoppeld zijn aan equipment, jobs, of andere objecten. Filter op DocumentType, naam, of gekoppeld object.`,
    ...
    async (input) => {
      try {
        const result = await getObjects("Document", buildOData(input));
  ...
  server.tool(
    ...
    async (input) => {
      try {
        const result = await getObject("Document", input.id, { select: input.select, expand: input.expand });
        return success(result);
      } catch (e: any) {
        return error(e.message);
      }
  ...
  server.tool(
    ...
    async (input) => {
      try {
        const result = await createObject("Document", input.data);
        return success(result);
      } catch (e: any) {
        return error(e.message);
      }
  ...
  server.tool(
    ...
    async (input) => {
      try {
        const result = await getObjects("DocumentType", buildOData(input));
        return success(result);
      } catch (e: any) {
        return error(e.message);
      }
...
(equipment, jobs, etc.) gekoppeld zijn.`,
    ...
    async (input) => {
      try {
        const result = await getObjects("ObjectDocument", buildOData(input));
        return success(result);
      } catch (e: any) {
        return error(e.message);
      }
...
Gebruik dit om foto's (bijv. van storingen) aan een job te koppelen via de API.`,
    ...
    async (input) => {
      try {
        const result = await executeAction("REST_AttachImageToJob", input.data);
        return success(result);
      } catch (e: any) {
        return error(e.message);
      }
